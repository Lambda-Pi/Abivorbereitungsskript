\documentclass[a4paper,11pt]{scrreprt}

\usepackage[ngerman]{babel}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{graphicx}
\usepackage{amsmath,amssymb,amsthm}
\usepackage[tracking=true]{microtype}
\usepackage{lmodern}
\usepackage{color}
\usepackage{hyperref}

% ToDo Notes einbinden
\input{Defines/ToDos.tex}

\title{Skript zum Mathe - Abi-Vorbereitungskurs~2015\\
\includegraphics[scale=0.15]{Images/Logo.jpg}
\author{Jan, Lucas \& Patrick\\
www.lambda-pi.de}
\date{\today}}

\usepackage{amsmath}
\begin{document}

	% Titelseite einfügen
	\maketitle
	\newpage

	% Liste der ToDo's erzeugen
	\listoftodos
	
	% Inhaltsverzeichnis einf�gen
	\tableofcontents
	\newpage
	
	\todo[color=red]{In allen Teilen ToDo's für die Umstrukturierung einfügen}
	\todo[color=red]{Einzelne Sektionen mit kurzer Zusammenfassung}
	\todo[color=red]{Einzelne Sektionen mit Signalwörter}
	% Vorwort 
	\input{Chapters/Vorwort.tex}
	
	\todo[color=red]{Kapitel "Aufbau des Skripts" einfügen}
	\todo[color=red]{Kapitel "Strukturiertes lösen von Aufgaben" einfügen}
	
	% Wiederholungen und Vorgeplänkel
	\input{Chapters/Wiederholungen/_Wiederholungen}
	
	% Analysis
	\input{Chapters/Analysis/_Analysis.tex}
	
	% Analytische Geometrie (Vektoren)
	\input{Chapters/Geometrie/_Geometrie.tex}
	
	% Stochastik
	\input{Chapters/Stochastik/_Stochastik.tex}
	
	\todo[color=red]{"Anhang Taschenrechner" einfügen}
\end{document}