\subsection{Grundarten von Funktionen}
	...
	\subsubsection{Lineare Funktionen \& deren Normale}
		...
		\tags{
			Gerade, rechtwinklig (für Normale)
		}
	...
	% Funktionen, welche geteilt werden. HeiÃen unsere ganzrationalen Funktionen
	% f(x) und g(x), so ist unsere gebrochene rationale Funktion:
	%\[h(x)=\frac{f(x)}{g(x)}\]
	%Hier gilt nur zu beachten, dass es einen Grad im Nenner und einen im ZÃ¤hler
	% gibt und dass sie im Gegensatz zu den ganzrationalen Funktionen eine
	% DefinitionslÃ¼cke haben kÃ¶nnen und zwar immer dann, wenn im Nenner 0 heraus
	...
	\subsubsection{Exponentiale Funktionen}
		...
		%Nun noch zu den Logarithmusfunktionen. Wie schon erwÃ¤hnt, sind diese die
		% Umkehrfunktionen der e-Funktion\footnote{Was genau das heiÃt, sieht man
		% gerade bei den beiden Funktionen sehr schÃ¶n, wenn man beide Zeichnen
		% lÃ¤sst.
		% Abstrakter kann man sich das so klar machen: Haben wir die e-Funktion, so
		% vertauschen wir x und y einfach und lÃ¶sen wieder nach y=\ldots auf.}.
		% Darstellen lassen sie sich als: \[f(x)=a\cdot ln(b(x-c))+d\] a ist wieder
		% die Streckung, c und d die Verschiebung und b ist Ã¤hnlich geartet wie das b
	...
	\subsubsection{Trigonometrische Funktionen}
		...
		\paragraph{Bogenmaß}
			...
			\summary{
				Das Bogenmaß ist eine mathematischere Darstellung für einen Winkel als
				der Grad.
			}
